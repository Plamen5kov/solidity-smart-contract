...
describe('Lottery ticket', function () {
    ...
    this.beforeAll(async () => {
        ...
        await create2FactoryContract.deployGame(ethers.utils.formatBytes32String(salt), currentBlockNumber, currentBlockNumber + 8)
    });
    ...
    it('Run a lottery and make sure half the reward stays in the contract after a valid winner is chosen and funds have been transfered', async function () {
        ...
        expect(await create2FactoryContract.getBalance()).to.equal(ethers.BigNumber.from("22000000000000000"))
        expect(lotteryWinner.gt(ethers.BigNumber.from("10000000000000000000000"))).to.eq(true, "someone must be a winner")
    });
    ...
    it('Once lottery has ended no more transactions are accepted', async function () {
        ...
        await create2FactoryContract.connect(ticketHolder1).buyTicket(options)
        await expect(create2FactoryContract.connect(ticketHolder2).buyTicket(options)).to.be.revertedWith("Lottery Ended")
    });
});